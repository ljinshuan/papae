...
- [x] 4.7 编写 test_pose_segmentor.py：测试单人物、多人物、无人、置信度过滤
- [x] 4.8 运行测试和 basedpyright，确保通过
...
- [x] 7.7 编写 test_visualizer.py：测试标注正确性、输出文件存在
- [x] 7.8 运行测试和 basedpyright，确保通过
...
- [x] 8.6 编写 test_report_generator.py：测试报告结构、关键帧嵌入、免责声明
- [x] 8.7 运行测试和 basedpyright，确保通过
...
- [x] 9.9 编写 test_cli.py：测试参数解析、成功流程、各种失败场景
- [x] 9.10 运行测试和 basedpyright，确保通过

## 10. 端到端集成与验证

- [x] 10.1 准备测试视频：2-3 段公开儿童走路视频放入 tests/fixtures/
- [x] 10.2 运行完整流水线：从 CLI 到报告+可视化视频的全流程
- [x] 10.3 验证报告内容：结构正确、关键帧嵌入、免责声明存在
- [x] 10.4 验证可视化视频：骨架连线、mask 叠加、关键帧标记正确
- [x] 10.5 验证错误场景：过短视频、模糊视频、无人视频、无效 API 密钥
- [x] 10.6 运行全部测试套件：pytest，确保 100% 通过
- [x] 10.7 运行静态分析：basedpyright，确保无类型错误
- [x] 10.8 编写 README.md：安装说明、使用方法、拍摄建议、免责声明
